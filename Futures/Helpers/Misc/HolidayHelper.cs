...
internal static class HolidayHelper
{
    public static HashSet<DateOnly> GetHolidays()
    {
        ...
        var minTradeDate = TradeDate.MinValue;

        for (var year = minTradeDate.Year;
            year <= TradeDate.MaxValue.Year; year++)
        {
            dates.AddRange(GetHolidaysForYear(year, minTradeDate));
        }

        return new HashSet<DateOnly>(dates);
    }

    private static List<DateOnly> GetHolidaysForYear(
        int year, DateOnly minTradeDate)
    {
        ...
        return holidays.Where(h => h >= minTradeDate).ToList();
    }
}